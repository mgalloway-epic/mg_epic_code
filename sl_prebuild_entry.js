...
function (record, search, runtime, log, url) {
     ...
     * Governance: flat cost of 3 searches regardless of lot count.
     * Uses .each() throughout so all results are returned (up to 4,000).
     */
    function getLotsForItem(itemId, locationId) {
        const lots = [];
        try {
            // Step 1: get ALL on-hand lots
            const lotFilters = [
                ['item', 'anyof', itemId],
                'AND',
            ...
            // Step 2: batch bin-name lookup for ALL lots
            try {
                const binBalFilters = [
                ...
                // Step 3: batch bin-ID lookup for unique bin names
